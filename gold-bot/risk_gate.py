...
# ИЗМЕРЕНО на реальном счету Али (скриншот "Мои комиссии" -> Фьючерсы,
# 11.07.2026, со включённой скидкой MNT 10%). БЕЗ скидки MNT было бы
# Taker 0.10% / Maker 0.036% - если скидка когда-нибудь отключится
# (например, кончится баланс MNT), эти числа нужно будет пересчитать заново.
TAKER_FEE_PCT = 0.0900 / 100      # комиссия taker (вход и выход - оба маркет-ордера в бэктесте)
MAKER_FEE_PCT = 0.0324 / 100      # используется только в отдельном эксперименте compare_maker_fee.py
