...
# тянут вверх редкие дни-выбросы (единичный гэп, разовое событие), а не стабильно
# высокая волатильность каждый день.
OUTLIER_RATIO = 2.0

# Если МЕДИАНА часа выше медианы медиан по всем остальным часам в CROSS_HOUR_RATIO
# раз - подозреваем системный артефакт, повторяющийся почти каждый день именно в
# этот час (например, технический скачок тиков на границе торгового дня у брокера),
# а не просто редкий выброс. Это ловит то, что OUTLIER_RATIO не ловит: когда
# аномалия происходит регулярно, а не в паре дней - тогда среднее и медиана внутри
# часа растут вместе, и соотношение "среднее/медиана" само по себе не выглядит подозрительно.
CROSS_HOUR_RATIO = 2.5
...
def main():
    ...
    # Два независимых признака аномального часа:
    # 1) внутри часа среднее сильно выше медианы - несколько дней-выбросов тянут среднее вверх;
    # 2) медиана часа сильно выше медиан остальных часов - аномалия регулярная, а не разовая.
    baseline_median = stats["median_range_usd"].median()
    is_mean_skewed = stats["avg_range_usd"] > OUTLIER_RATIO * stats["median_range_usd"]
    is_cross_hour_spike = stats["median_range_usd"] > CROSS_HOUR_RATIO * baseline_median
    suspicious = stats[is_mean_skewed | is_cross_hour_spike]

    if not suspicious.empty:
        print("\n" + "!" * 70)
        print("ВНИМАНИЕ: эти часы выглядят аномально - либо среднее раздуто редкими")
        print("выбросами, либо у часа регулярно (не только в отдельные дни) медиана в")
        print(f"{CROSS_HOUR_RATIO:.1f}+ раз выше типичной по остальным часам ({baseline_median:.2f}$).")
        print("Похоже на недельный гэп открытия рынка или технический артефакт смены")
        print("торгового дня у брокера, а не на честную внутридневную волатильность:")
