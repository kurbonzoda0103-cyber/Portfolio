...
позиции считается по эффективному плечу (то же MAX_EFFECTIVE_LEVERAGE), БЕЗ
...
где ценовой стоп не имеет смысла (цена не создаёт риска, funding - создаёт).
...
def size_position(equity_usdt: float) -> float:
    """Дельта-нейтральная позиция не имеет ценового стопа (движение цены не
    создаёт риска - спот и перп компенсируют друг друга), поэтому размер
    считается просто через эффективное плечо, а не risk_gate.compute_position_size
    (той функции нужна стоп-дистанция, здесь её нет и не может быть)."""
    return risk_gate.MAX_EFFECTIVE_LEVERAGE * equity_usdt
