      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
            <TrendingUp className="text-red-400" /> Тренды
          </h1>
          <p className="text-white/40">Топ видео прямо сейчас на YouTube</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {videos.map((v) => (
            <a
              ...
              className="glass rounded-xl overflow-hidden hover:border-white/15 transition-all group"
            >
              {v.thumbnailUrl ? (
              ...
              <div className="p-4">
                <p className="text-white font-semibold text-sm leading-tight line-clamp-2 mb-2 group-hover:text-red-300 transition-colors">
                  {v.title}
                </p>
                ...
                <div className="flex items-center gap-4 text-white/50 text-xs">
                  ...
                  <span className="ml-auto text-white/30">{v.category}</span>
                </div>
              </div>
            </a>
          ))}
